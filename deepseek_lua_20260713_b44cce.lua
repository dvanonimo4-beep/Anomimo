--[[ ABD HUB – PAINEL COMPACTO PARA CELULAR ]]
-- Tamanho reduzido, com rolagem e botões grandes para toque
...
print("🚀 Carregando Abd Hub (Mobile)...")
...
-- =============================================
-- CORES (SUAVES PARA MOBILE)
-- =============================================
local Cores = {
    Fundo = Color3.fromRGB(8, 8, 20),
    FundoClaro = Color3.fromRGB(16, 16, 35),
    Borda = Color3.fromRGB(0, 180, 255),
    Texto = Color3.fromRGB(255, 255, 255),
    TextoClaro = Color3.fromRGB(170, 170, 200),
    Verde = Color3.fromRGB(0, 180, 70),
    Vermelho = Color3.fromRGB(180, 40, 40),
    Amarelo = Color3.fromRGB(230, 160, 0),
    Azul = Color3.fromRGB(0, 130, 230),
    Destaque = Color3.fromRGB(0, 220, 255),
}

-- =============================================
-- BOLINHA FLUTUANTE (PEQUENA)
-- =============================================
local floatBtn = Instance.new("TextButton")
floatBtn.Size = UDim2.new(0, 50, 0, 50)
floatBtn.Position = UDim2.new(0.02, 0, 0.80, 0)
...
panel.Position = UDim2.new(0.5, -150, 0.35, -190)
...
abaContainer.Size = UDim2.new(1, 0, 0, 28)
abaContainer.Position = UDim2.new(0, 0, 0, 32)
...
local function criarAba()
    ...
    f.Position = UDim2.new(0, 0, 0, 64)
    f.BackgroundTransparency = 1
    f.Visible = false
    f.CanvasSize = UDim2.new(0, 0, 0, 0)
    f.ScrollBarThickness = 3
    f.ScrollBarImageColor3 = Cores.Borda
    f.Parent = panel
    return f
end
...
for i, btn in ipairs(botoesAba) do
    btn.MouseButton1Click:Connect(function()
        ...
        -- Ajusta o CanvasSize após trocar de aba
        task.wait(0.05)
        if conteudosAba[i] and conteudosAba[i]:IsA("ScrollingFrame") then
            local content = conteudosAba[i]
            local maxY = 0
            ...
            content.CanvasSize = UDim2.new(0, 0, 0, maxY + 40)
        end
    end)
end
...
local function criarToggle(parent, texto, x, y, w, h, cor, callback)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        btn.BackgroundColor3 = state and Color3.fromRGB(40, 130, 40) or cor
...
local function criarSlider(parent, texto, x, y, w, h, min, max, default, callback)
    ...
    label.Size = UDim2.new(0.45, 0, 0.5, 0)
    ...
    menos.Size = UDim2.new(0, 24, 0, 20)
...
-- =============================================
local yOffset = 0
local function addY(dy)
    yOffset = yOffset + dy
    return yOffset
end

-- Velocidade
criarSecao(abaGeral, "⚡ VELOCIDADE", addY(0))
local speedLabel = Instance.new("TextLabel")
speedLabel.Size = UDim2.new(0.3, 0, 0, 22)
speedLabel.Position = UDim2.new(0.05, 0, addY(0.06), 0)
...
speedBtns.Size = UDim2.new(0.5, 0, 0, 24)
speedBtns.Position = UDim2.new(0.45, 0, addY(0.06), 0)
speedBtns.BackgroundTransparency = 1
speedBtns.Parent = abaGeral

criarBotao(speedBtns, "−", 0, 0, 38, 22, Cores.Vermelho, function()
    Config.Velocidade = math.max(16, Config.Velocidade - 5)
    speedLabel.Text = Config.Velocidade
    applySpeed()
end)
criarBotao(speedBtns, "+", 0.42, 0, 38, 22, Cores.Verde, function()
    Config.Velocidade = math.min(100, Config.Velocidade + 5)
    speedLabel.Text = Config.Velocidade
    applySpeed()
end)
criarBotao(speedBtns, "↺", 0.84, 0, 38, 22, Cores.Amarelo, function()
...
-- ESP
criarSecao(abaGeral, "👁️ ESP", addY(0))
criarToggle(abaGeral, "ESP", 0.05, addY(0.06), 100, 24, Color3.fromRGB(35, 35, 50), function(v)
    Config.ESP = v
    if v then startESP() else stopESP() end
end)
criarToggle(abaGeral, "NOCLIP", 0.52, addY(0.06), 100, 24, Color3.fromRGB(35, 35, 50), function(v)
    Config.Noclip = v
    applyNoclip()
end)
addY(0.12)
...
-- Defesa
criarSecao(abaGeral, "🛡️ DEFESA", addY(0))
criarToggle(abaGeral, "ANTI-KB", 0.05, addY(0.06), 120, 24, Color3.fromRGB(35, 35, 50), function(v)
    Config.AntiKnockback = v
    if v then startAntiKB() elseif antiKBLoop then antiKBLoop:Disconnect() end
end)
criarToggle(abaGeral, "GODMODE", 0.52, addY(0.06), 100, 24, Color3.fromRGB(35, 35, 50), function(v)
    Config.Godmode = v
...
-- Regeneração
criarSecao(abaGeral, "🩸 REGEN", addY(0))
criarToggle(abaGeral, "REGEN", 0.05, addY(0.06), 100, 24, Color3.fromRGB(35, 35, 50), function(v)
    Config.Regen = v
    if v then startRegen() elseif regenLoop then regenLoop:Disconnect() end
end)
criarSlider(abaGeral, "Vel", 0.52, addY(0.06), 120, 24, 1, 20, 5, function(v)
    Config.RegenSpeed = v
end)
addY(0.12)
...
criarSecao(abaJogo, "⚡ AUTO GEN", addY(0))
criarToggle(abaJogo, "GEN", 0.05, addY(0.06), 100, 24, Color3.fromRGB(35, 35, 50), function(v)
    Config.AutoGenerator = v
    if v then startAutoGen() else stopAutoGen() end
end)
criarToggle(abaJogo, "TURBO", 0.52, addY(0.06), 100, 24, Color3.fromRGB(35, 35, 50), function(v)
    Config.TurboMode = v
end)
addY(0.10)

criarBotao(abaJogo, "Ótimo", 0.05, addY(0), 60, 22, Cores.Verde, function()
    Config.GeneratorMode = "great"
    genLabel.Text = "Ótimo"
end)
criarBotao(abaJogo, "Normal", 0.38, addY(0), 60, 22, Cores.Amarelo, function()
    Config.GeneratorMode = "normal"
    genLabel.Text = "Normal"
end)
local genLabel = Instance.new("TextLabel")
genLabel.Size = UDim2.new(0.2, 0, 0, 22)
genLabel.Position = UDim2.new(0.68, 0, addY(0), 0)
...
criarSlider(abaJogo, "Dist", 0.05, addY(0), 220, 24, 5, 50, 25, function(v)
    Config.AutoGeneratorRange = v
end)
addY(0.10)

criarToggle(abaJogo, "SKILL PERF", 0.05, addY(0), 170, 24, Color3.fromRGB(35, 35, 50), function(v)
    Config.SkillCheckManual = v
    if v then setupSkillCheck() end
end)
addY(0.10)

criarSecao(abaJogo, "📦 AÇÕES", addY(0))
criarBotao(abaJogo, "COMPLETAR TUDO", 0.05, addY(0.06), 200, 26, Cores.Azul, function()
    completeAllGenerators()
end)
addY(0.12)

criarSecao(abaJogo, "🚪 FUGA", addY(0))
criarToggle(abaJogo, "AUTO LEAVE", 0.05, addY(0.06), 100, 24, Color3.fromRGB(35, 35, 50), function(v)
    Config.AutoLeave = v
    if v then startAutoLeave() else stopAutoLeave() end
end)
criarSlider(abaJogo, "Dist", 0.52, addY(0.06), 120, 24, 5, 30, 15, function(v)
    Config.LeaveDistance = v
end)
addY(0.12)

criarSecao(abaJogo, "⚔️ AUTO ATK", addY(0))
criarToggle(abaJogo, "ATK", 0.05, addY(0.06), 80, 24, Color3.fromRGB(35, 35, 50), function(v)
    Config.AutoAttack = v
    if v then startAutoAttack() else stopAutoAttack() end
end)
criarSlider(abaJogo, "Alc", 0.40, addY(0.06), 70, 24, 5, 20, 10, function(v)
    Config.AttackRange = v
end)
criarSlider(abaJogo, "CD", 0.75, addY(0.06), 70, 24, 0.5, 2.0, 1.2, function(v)
    Config.AttackCooldown = v
end)
addY(0.12)

-- =============================================
-- ABA PROTEÇÃO (COMPACTA)
-- =============================================
yOffset = 0

criarSecao(abaProtecao, "♾️ IMORTAL", addY(0))
criarToggle(abaProtecao, "GODMODE", 0.05, addY(0.06), 200, 26, Color3.fromRGB(35, 35, 50), function(v)
    Config.Godmode = v
    if v then startGodmode() elseif godLoop then godLoop:Disconnect() end
end)
addY(0.12)

criarSecao(abaProtecao, "🩸 REGEN", addY(0))
criarToggle(abaProtecao, "REGEN", 0.05, addY(0.06), 100, 24, Color3.fromRGB(35, 35, 50), function(v)
    Config.Regen = v
    if v then startRegen() elseif regenLoop then regenLoop:Disconnect() end
end)
criarSlider(abaProtecao, "Vel", 0.52, addY(0.06), 120, 24, 1, 20, 5, function(v)
    Config.RegenSpeed = v
end)
addY(0.12)

criarSecao(abaProtecao, "🛡️ DEFESA", addY(0))
criarToggle(abaProtecao, "ANTI-KB", 0.05, addY(0.06), 200, 26, Color3.fromRGB(35, 35, 50), function(v)
    Config.AntiKnockback = v
    if v then startAntiKB() elseif antiKBLoop then antiKBLoop:Disconnect() end
end)
addY(0.12)

criarSecao(abaProtecao, "🚪 NOCLIP", addY(0))
criarToggle(abaProtecao, "NOCLIP", 0.05, addY(0.06), 200, 26, Color3.fromRGB(35, 35, 50), function(v)
    Config.Noclip = v
    applyNoclip()
end)
addY(0.12)
...
-- =============================================
-- AJUSTAR CANVAS SIZE PARA ROLAGEM
-- =============================================
local function ajustarCanvas(aba)
    task.wait(0.05)
    if not aba or not aba:IsA("ScrollingFrame") then return end
    local maxY = 0
    for _, child in ipairs(aba:GetChildren()) do
        if child:IsA("Frame") or child:IsA("TextButton") or child:IsA("TextLabel") then
            local pos = child.Position.Y.Offset + child.Size.Y.Offset
            if pos > maxY then maxY = pos end
        end
    end
    aba.CanvasSize = UDim2.new(0, 0, 0, maxY + 40)
end

-- Ajusta todas as abas
for _, aba in ipairs(conteudosAba) do
    ajustarCanvas(aba)
end

-- =============================================
-- FUNÇÕES DE PROTEÇÃO (MESMAS DE ANTES)
-- =============================================
...
print("========================================")
print("✅ ABD HUB – MOBILE COMPACTO")
print("📱 Tamanho reduzido para celular")
print("📜 Rolagem nas abas")
print("⚔️ Toque na bolinha para abrir")
print("========================================")