--[[ ABD HUB – Violence District (Versão Final) ]]
-- Removido: Configurações Teleporte, Anti-Morte, Anti-Queda
-- Adicionado: Skill Check Perfeito, Indicador de Gerador Completo, Anti-Câmera Tremida, Auto-Farm Turbo
...
print("=== ABD HUB – Violence District (Final) ===")
...
local Config = {
    ...
    AutoFeatures = {
        ...
        AttackRange = 10,
        AttackCooldown = 1.2,
        PerfectSkillCheck = false,  -- NOVO
        TurboMode = false,          -- NOVO
    },
    Teleportation = {
        TeleportOffset = 3,
        SafeTeleport = true,
...
local lastGenCompleteNotify = 0

local myChar = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
local regenLoop, godLoop, antiKBLoop
...
-- =============================================
-- FUNÇÕES DE PROTEÇÃO (SEM ANTI-MORTE E ANTI-QUEDA)
-- =============================================
...
-- =============================================
-- AUTO GENERATOR (COM SKILL CHECK PERFEITO E TURBO MODE)
-- =============================================
local function getGeneratorRemotes()
    local remotes = ReplicatedStorage:FindFirstChild("Remotes")
    if not remotes then remotes = ReplicatedStorage end
    local genFolder = remotes:FindFirstChild("Generator") or remotes:FindFirstChild("Generators")
    if not genFolder then return nil, nil end
    local repair = genFolder:FindFirstChild("RepairEvent") or genFolder:FindFirstChild("Repair")
    local skill = genFolder:FindFirstChild("SkillCheckResultEvent") or genFolder:FindFirstChild("SkillCheck")
    return repair, skill
end

local function autoGeneratorLoop()
    if not Config.AutoFeatures.AutoGenerator then return end
    ...
    if nearestDist > Config.AutoFeatures.AutoGeneratorRange then return end
    if not nearestGen then return end
    
    local repair, skill = getGeneratorRemotes()
    if not repair or not skill then return end
    
    -- Determina quantas vezes disparar (Turbo Mode)
    local repeats = Config.AutoFeatures.TurboMode and 3 or 1
    
    for _, point in ipairs(nearestGen:GetChildren()) do
        if point.Name:find("GeneratorPoint") then
            pcall(function()
                for i = 1, repeats do
                    repair:FireServer(point, true)
                    
                    -- Skill Check Perfeito ou Normal
                    local result = Config.AutoFeatures.PerfectSkillCheck and "success" or Config.AutoFeatures.GeneratorMode == "great" and "success" or "neutral"
                    local value = Config.AutoFeatures.PerfectSkillCheck and 1 or (Config.AutoFeatures.GeneratorMode == "great" and 1 or 0)
                    
                    skill:FireServer(result, value, nearestGen, point)
                end
            end)
        end
    end
...
local function startAutoGenLoop()
    ...
    AutoGenConnection = RunService.Heartbeat:Connect(autoGeneratorLoop)
end
...
local function completeAllGenerators()
    ...
    local repair, skill = getGeneratorRemotes()
    if not repair or not skill then
        notify("Erro", "Remotos não encontrados", 3)
        return
    end
    local completed = 0
    for _, obj in ipairs(map:GetDescendants()) do
    ...
    if completed > 0 then
        notify("Completo!", string.format("%d gerador(es) completos", completed), 4)
    else
        notify("Falha", "Nenhum gerador encontrado", 3)
    end
end

-- =============================================
-- INDICADOR DE GERADOR COMPLETO (NOTIFICAÇÃO)
-- =============================================
local function checkGeneratorComplete()
    local map = Workspace:FindFirstChild("Map")
    if not map then return end
    
    local now = tick()
    if now - lastGenCompleteNotify < 2 then return end -- Evita spam
    
    for _, obj in ipairs(map:GetDescendants()) do
        if obj:IsA("Model") and obj.Name == "Generator" then
            -- Tenta detectar se o gerador foi completado
            local completeAttr = obj:FindFirstChild("Complete") or obj:FindFirstChild("Done") or obj:FindFirstChild("Finished")
            if completeAttr then
                if completeAttr:IsA("BoolValue") and completeAttr.Value == true then
                    notify("Gerador Completo!", "Um gerador foi finalizado!", 2)
                    lastGenCompleteNotify = now
                    break
                end
            end
        end
...
RunService.Heartbeat:Connect(checkGeneratorComplete)
...
local function fixCamera()
    local camera = workspace.CurrentCamera
    if camera then
        -- Remove efeitos de trepidação forçando um estado estável
        camera.CameraType = Enum.CameraType.Scriptable
        camera.CameraType = Enum.CameraType.Custom
    end
end
RunService.Heartbeat:Connect(fixCamera)

-- =============================================
-- AUTO ATAQUE CORRIGIDO
-- =============================================
local function performAutoAttack()
    ...
    if hum and hum.WalkSpeed < 12 then
        return
    end
...
local function startAutoAttack()
    ...
    notify("Ataque Auto", "Alcance: " .. Config.AutoFeatures.AttackRange .. " cooldown: " .. Config.AutoFeatures.AttackCooldown .. "s", 3)
end
...
-- =============================================
-- CORREÇÃO DAS PALETAS
-- =============================================
local function isPalletDropped(pallet)
    -- Verifica atributo
    local droppedAttr = pallet:FindFirstChild("Dropped") or pallet:FindFirstChild("Destroyed") or pallet:FindFirstChild("Used")
    if droppedAttr and droppedAttr:IsA("BoolValue") and droppedAttr.Value == true then
        return true
    end
    
    -- Verifica rotação
    local primaryPart = pallet:FindFirstChild("PrimaryPart") or pallet:FindFirstChildWhichIsA("BasePart")
    if primaryPart then
        local rotX = math.abs(primaryPart.Orientation.X or 0)
        local rotZ = math.abs(primaryPart.Orientation.Z or 0)
        if rotX > 30 or rotZ > 30 then
            return true
        end
        if primaryPart.Position.Y < 1.5 then
            return true
        end
    end
    return false
...
local function fixPalletESP()
    local map = Workspace:FindFirstChild("Map")
    if not map then return end
    
    for _, obj in ipairs(map:GetDescendants()) do
        if obj:IsA("Model") and (obj.Name == "Palletwrong" or obj.Name:find("Pallet") or obj.Name:find("Palete")) then
            local isDropped = isPalletDropped(obj)
            if isDropped then
                removeHighlight(obj)
            else
...
RunService.Heartbeat:Connect(fixPalletESP)
...
local Window = Rayfield:CreateWindow({
    ...
    ConfigurationSaving = {Enabled = true, FolderName = nil, FileName = "AbdHubFinal"},
...
ESPTab:CreateLabel("🟡 Paletas em pé são destacadas automaticamente em amarelo.")

-- Aba Jogo
local GameplayTab = Window:CreateTab("🎮 Jogo", 4483362458)
GameplayTab:CreateSection("Auto Gerador")
GameplayTab:CreateToggle({Name = "Auto Completar Geradores", CurrentValue = false, Flag = "AutoGenerator", Callback = function(v)
    Config.AutoFeatures.AutoGenerator = v
    if v then startAutoGenLoop() else stopAutoGenLoop() end
    notify("Auto Gerador", v and "Ativado" or "Desativado", 2)
end})
GameplayTab:CreateDropdown({Name = "Modo Gerador", Options = {"Ótimo (Rápido)", "Normal (Lento)"}, CurrentOption = "Ótimo (Rápido)", Flag = "GeneratorMode", Callback = function(o)
    Config.AutoFeatures.GeneratorMode = o:find("Ótimo") and "great" or "normal"
end})
GameplayTab:CreateSlider({Name = "Distância Máxima para Atuar", Range = {5, 50}, Increment = 1, CurrentValue = 25, Flag = "AutoGeneratorRange", Callback = function(v)
    Config.AutoFeatures.AutoGeneratorRange = v
end})
GameplayTab:CreateToggle({Name = "Skill Check Perfeito", CurrentValue = false, Flag = "PerfectSkillCheck", Callback = function(v)
    Config.AutoFeatures.PerfectSkillCheck = v
    notify("Skill Check", v and "Perfeito ativado" or "Normal", 2)
end})
GameplayTab:CreateToggle({Name = "Modo Turbo (Farm Rápido)", CurrentValue = false, Flag = "TurboMode", Callback = function(v)
    Config.AutoFeatures.TurboMode = v
    notify("Turbo Mode", v and "Ativado (3x mais rápido)" or "Desativado", 2)
end})
GameplayTab:CreateLabel("⚡ Turbo Mode acelera o Auto Generator.")

GameplayTab:CreateSection("Ações Manuais")
GameplayTab:CreateButton({Name = "Completar Todos Geradores (Instantâneo)", Callback = completeAllGenerators})

GameplayTab:CreateSection("Fuga Rápida")
GameplayTab:CreateToggle({Name = "Sair Gerador Rápido", CurrentValue = false, Flag = "AutoLeaveGenerator", Callback = function(v)
    Config.AutoFeatures.AutoLeaveGenerator = v
    if v then startAutoLeaveGenerator() else stopAutoLeaveGenerator() end
end})
if not isMobile then
    GameplayTab:CreateDropdown({Name = "Tecla para Sair", Options = {"Q","E","F","G","X","Z","V","B"}, CurrentOption = "Q", Flag = "LeaveKeybind", Callback = function(o)
        local map = {Q=Enum.KeyCode.Q, E=Enum.KeyCode.E, F=Enum.KeyCode.F, G=Enum.KeyCode.G, X=Enum.KeyCode.X, Z=Enum.KeyCode.Z, V=Enum.KeyCode.V, B=Enum.KeyCode.B}
        Config.AutoFeatures.LeaveKeybind = map[o]
        if Config.AutoFeatures.AutoLeaveGenerator then stopAutoLeaveGenerator(); startAutoLeaveGenerator() end
    end})
end
GameplayTab:CreateSlider({Name = "Distância de Detecção", Range = {5,30}, Increment = 1, CurrentValue = 15, Flag = "LeaveDistance", Callback = function(v)
    Config.AutoFeatures.LeaveDistance = v
end})
...
GameplayTab:CreateSlider({Name = "Alcance do Ataque", Range = {5,20}, Increment = 1, CurrentValue = 10, Flag = "AttackRange", Callback = function(v)
    Config.AutoFeatures.AttackRange = v
end})
GameplayTab:CreateSlider({Name = "Cooldown do Ataque (s)", Range = {0.5, 2.0}, Increment = 0.1, CurrentValue = 1.2, Flag = "AttackCooldown", Callback = function(v)
    Config.AutoFeatures.AttackCooldown = v
end})
GameplayTab:CreateLabel("⚡ Aumente o cooldown para evitar ficar lento.")
...
-- Aba Teleporte (SEM CONFIGURAÇÕES)
...
-- Aba Proteção (SEM ANTI-MORTE E ANTI-QUEDA)
local MyTab = Window:CreateTab("🛡️ Proteção", 4483362458)
MyTab:CreateSection("⚡ Velocidade")
MyTab:CreateSlider({Name = "Velocidade (16-100)", Range = {16, 100}, Increment = 1, CurrentValue = 16, Flag = "SpeedSlider", Callback = function(v)
    Config.MyFeatures.Speed = v
    applySpeed()
end})
MyTab:CreateSection("🚪 Atravessar")
MyTab:CreateToggle({Name = "Atravessar Paredes (Noclip)", CurrentValue = false, Flag = "NoclipToggle", Callback = function(v)
    Config.MyFeatures.Noclip = v
    applyNoclip()
end})
MyTab:CreateSection("♾️ Deus")
MyTab:CreateToggle({Name = "Deus (Vida Infinita)", CurrentValue = false, Flag = "GodmodeToggle", Callback = function(v)
    Config.MyFeatures.Godmode = v
    if v then startGodmode() elseif godLoop then godLoop:Disconnect() end
end})
MyTab:CreateSection("🩸 Regeneração")
MyTab:CreateToggle({Name = "Regeneração Automática", CurrentValue = false, Flag = "RegenToggle", Callback = function(v)
    Config.MyFeatures.Regen = v
    if v then startRegen() elseif regenLoop then regenLoop:Disconnect() end
end})
MyTab:CreateSlider({Name = "Velocidade da Regeneração (1-20 HP/s)", Range = {1, 20}, Increment = 1, CurrentValue = 5, Flag = "RegenSpeedSlider", Callback = function(v)
    Config.MyFeatures.RegenSpeed = v
end})
MyTab:CreateSection("🛡️ Anti-Knockback")
MyTab:CreateToggle({Name = "Anti-Knockback (Não Ser Empurrado)", CurrentValue = false, Flag = "AntiKBToggle", Callback = function(v)
    Config.MyFeatures.AntiKnockback = v
...
notify("Abd Hub", "Carregado com sucesso! (Versão Final)", 4)
print("========================================")
print("✅ ABD HUB – VIOLENCE DISTRICT (FINAL)")
print("📱 Mobile:", isMobile and "SIM" or "NÃO")
print("🗑️ Removido: Anti-Morte, Anti-Queda, Config Teleporte")
print("✅ Adicionado: Skill Check Perfeito, Turbo Mode")
print("✅ Adicionado: Indicador de Gerador Completo")
print("✅ Adicionado: Anti-Câmera Tremida")
print("========================================")