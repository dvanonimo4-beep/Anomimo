--[[ ABD HUB – VERSÃO LIMPA (SEM BOTÕES EXTRAS E ESP SIMPLES) ]]
...
local function iniciarESP()
    ...
    notificar("ESP Iniciado", "Apenas cores (Assassino=Vermelho | Sobrevivente=Verde)", 2)
end
...
local function pararESP()
    ...
    limparESP()
    notificar("ESP Parado", "Todos os destaques removidos", 2)
end

-- =============================================
-- FUNÇÕES DO JOGO (GERADOR, TELEPORTE, ATAQUE)
-- =============================================
...
function sairGerador()
    ...
    if not ng or nd > C.Auto.DistanciaSair then
        notificar("Longe", "Você não está perto de um gerador", 2)
        return false
    end
...
local function iniciarSairGerador()
    ...
    if not isMobile then
        ...
        notificar("Sair Gerador", "Pressione " .. C.Auto.TeclaSair.Name .. " para sair", 3)
    else
        notificar("Modo Mobile", "Use o botão no menu 'Jogo' para sair", 3)
    end
end
...
local function iniciarAtaqueAuto()
    ...
    if not ehAssassino() then
        notificar("Erro", "Você precisa ser o Assassino!", 3)
        return
    end
...
-- =============================================
-- INTERFACE RAYFIELD (SEM BOTÕES MOBILE EXTRAS)
-- =============================================
local Janela = Rayfield:CreateWindow({
    Nome = "🎮 Abd Hub",
    TituloCarregamento = "Abd Hub",
    SubtituloCarregamento = "Versão Limpa",
...
AbaESP:CriarSecao("ESP Jogadores (Somente Cores)")
AbaESP:CriarAlternar({
    ...
    Callback = function(v)
        C.ESP.Assassino = v
        if v or C.ESP.Sobrevivente then
            iniciarESP()
        else
            pararESP()
        end
...
AbaJogo:CriarAlternar({
    Nome = "Habilitar Sair Gerador Rápido",
    ValorAtual = false,
...
if not isMobile then
    AbaJogo:CriarDropdown({
        Nome = "Tecla para Sair",
        Opcoes = {"Q", "E", "F", "G", "X", "Z", "V", "B"},
        OpcaoAtual = "Q",
        Bandeira = "TeclaSair",
        Callback = function(o)
            local m = {Q = Enum.KeyCode.Q, E = Enum.KeyCode.E, F = Enum.KeyCode.F, G = Enum.KeyCode.G,
                       X = Enum.KeyCode.X, Z = Enum.KeyCode.Z, V = Enum.KeyCode.V, B = Enum.KeyCode.B}
            C.Auto.TeclaSair = m[o]
            if C.Auto.AutoSairGerador then
                pararSairGerador()
                iniciarSairGerador()
            end
        end
...
AbaJogo:CriarControleDeslizante({
    Nome = "Distância de Detecção",
    Intervalo = {5, 30},
    Incremento = 1,
    ValorAtual = 15,
    Bandeira = "DistanciaSair",
    Callback = function(v) C.Auto.DistanciaSair = v end
})
AbaJogo:CriarBotao({
    Nome = "Sair Gerador Agora",
    Callback = sairGerador
})
AbaJogo:CriarSecao("Ações Manuais")
AbaJogo:CriarBotao({
    Nome = "Completar Todos Geradores (Instantâneo)",
    Callback = function()
        local mapa = W:FindFirstChild("Map")
        if not mapa then
            notificar("Erro", "Mapa não encontrado", 3)
            return
        end
        local completos = 0
        chamarSeguro(function()
        ...
        if completos > 0 then
            notificar("Completo!", string.format("%d gerador(es) completos", completos), 4)
        else
            notificar("Falha", "Nenhum gerador encontrado", 3)
        end
    end
})
AbaJogo:CriarSecao("Poderes do Assassino")
AbaJogo:CriarAlternar({
    Nome = "Auto Atacar Sobreviventes Próximos",
    ValorAtual = false,
    Bandeira = "AutoAtaque",
    Callback = function(v)
        C.Auto.AutoAtaque = v
            ...
            if remotes then
                ...
                if kr then
                    ...
                    if kf then
                        ...
                        if ap then
                            ap:FireServer()
                            notificar("Poder Ativado", "Poder do assassino acionado", 2)
                        end
                    end
                end
            end
        ...
        chamarSeguro(function()
            ...
            if remotes then
                ...
                if ataques then
                    ...
                    if ba then
                        ba:FireServer(false)
                        notificar("Ataque", "Ataque básico executado", 2)
                    end
                end
            end
        end)
...
AbaTeleporte:CriarBotao({
    ...
    Callback = function()
        ...
        if #gens == 0 then
            notificar("Não Encontrado", "Nenhum gerador no mapa", 3)
            return
        end
        if teleporteSeguro(gens[1].parte.CFrame) then
            notificar("Teleportado!", string.format("Mais próximo (%.0fm)", gens[1].distancia), 3)
        end
    end
})
AbaTeleporte:CriarBotao({
    ...
    Callback = function()
        ...
        if teleporteSeguro(far.parte.CFrame) then
            notificar("Teleportado!", string.format("Mais distante (%.0fm)", far.distancia), 3)
        end
    end
})
AbaTeleporte:CriarBotao({
    ...
    Callback = function()
        ...
        notificar("Iniciando", string.format("Teleportando por %d geradores...", #gens), 3)
        task.spawn(function()
            for i, gen in ipairs(gens) do
                if not pegarRaiz() then break end
                teleporteSeguro(gen.parte.CFrame)
                notificar("Gerador " .. i, string.format("%d/%d (%.0fm)", i, #gens, gen.distancia), 2)
                task.wait(C.Teleporte.DelayTeleporte)
            end
            notificar("Completo!", "Visitou todos os geradores", 3)
        end)
    end
})
...
AbaProtecao:CriarAlternar({
    Nome = "Atravessar Paredes (Noclip)",
    ValorAtual = false,
...
AbaProtecao:CriarControleDeslizante({
    Nome = "Velocidade da Regeneração (1-20 HP/s)",
    Intervalo = {1, 20},
    Incremento = 1,
    ValorAtual = 5,
...
AbaProtecao:CriarAlternar({
    Nome = "Anti-Knockback (Não Ser Empurrado)",
    ValorAtual = false,
...
AbaProtecao:CriarAlternar({
    Nome = "Anti-Queda (Não Cair / Ragdoll)",
    ValorAtual = false,
...
notificar("Abd Hub", "Carregado com sucesso! (Versão Limpa)", 4)

print("========================================")
print("✅ ABD HUB – Versão Limpa")
print("📱 Mobile:", isMobile and "SIM" or "NÃO")
print("👁️ ESP: Somente cores (sem textos)")
