...
-- Versão sem botões móveis extras (SAIR e TP GER)
...
local Config = {
    ...
    Mobile = {
        TouchControlsEnabled = false,  -- DESATIVADO (botões removidos)
        ButtonSize = 80,
...
-- =============================================
-- CONTROLES MÓVEIS (REMOVIDOS – SEM BOTÕES SAIR E TP)
-- =============================================
-- A função createMobileControls foi completamente removida.
-- Os botões "SAIR" e "TP GER" não serão mais criados.
...
-- A seção de Controles Mobile foi removida (não há mais botões SAIR e TP GER)
...
print("🔹 Botões 'SAIR' e 'TP GER' removidos")
