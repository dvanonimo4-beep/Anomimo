--[[ ABD HUB – PAINEL BONITO PARA CELULAR ]]
-- Layout limpo, organizado e com visual moderno
...
-- =============================================
-- CORES (MODERNAS E BONITAS)
-- =============================================
local Cores = {
    Fundo = Color3.fromRGB(12, 12, 28),
    FundoClaro = Color3.fromRGB(22, 22, 45),
...
-- =============================================
-- PAINEL PRINCIPAL (BONITO E ORGANIZADO)
-- =============================================
local panel = Instance.new("Frame")
panel.Size = UDim2.new(0, 320, 0, 400)
panel.Position = UDim2.new(0.5, -160, 0.32, -200)
...
abaContainer.Position = UDim2.new(0, 0, 0, 35)
...
for i, btn in ipairs(botoesAba) do
    btn.MouseButton1Click:Connect(function()
        ...
        if content and content:IsA("ScrollingFrame") then
            ...
            content.CanvasSize = UDim2.new(0, 0, 0, maxY + 50)
        end
    end)
end
...
local function criarToggle(parent, texto, x, y, w, h, cor, callback)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        btn.BorderColor3 = state and Color3.fromRGB(0, 255, 100) or Cores.Cinza
        callback(state)
    end)
    return btn, state
end
...
local function criarSlider(parent, texto, x, y, w, h, min, max, default, callback)
    ...
    menos.Position = UDim2.new(0.65, 0, 0.5, 0)
    ...
    mais.Position = UDim2.new(0.85, 0, 0.5, 0)
...
-- =============================================
-- ABA GERAL (ORGANIZADA)
-- =============================================
local y = 0
local function addY(dy) y = y + dy return y end

-- Velocidade
criarSecao(abaGeral, "⚡ VELOCIDADE", addY(0))
local speedFrame = Instance.new("Frame")
speedFrame.Size = UDim2.new(1, 0, 0, 30)
speedFrame.Position = UDim2.new(0, 0, addY(0.06), 0)
speedFrame.BackgroundTransparency = 1
speedFrame.Parent = abaGeral
...
criarBotao(speedFrame, "−", 0.30, 0, 40, 26, Cores.Vermelho, function()
    Config.Velocidade = math.max(16, Config.Velocidade - 5)
    speedLabel.Text = Config.Velocidade
    applySpeed()
end)
criarBotao(speedFrame, "+", 0.55, 0, 40, 26, Cores.Verde, function()
    Config.Velocidade = math.min(100, Config.Velocidade + 5)
    speedLabel.Text = Config.Velocidade
    applySpeed()
end)
criarBotao(speedFrame, "↺", 0.80, 0, 40, 26, Cores.Amarelo, function()
...
-- ESP e Noclip
criarSecao(abaGeral, "👁️ VISUAL", addY(0))
criarToggle(abaGeral, "ESP", 0.04, addY(0.06), 100, 26, Cores.FundoClaro, function(v)
    Config.ESP = v
    if v then startESP() else stopESP() end
end)
criarToggle(abaGeral, "NOCLIP", 0.52, addY(0.06), 100, 26, Cores.FundoClaro, function(v)
    Config.Noclip = v
    applyNoclip()
end)
addY(0.12)
...
criarToggle(abaGeral, "ANTI-KB", 0.04, addY(0.06), 120, 26, Cores.FundoClaro, function(v)
    Config.AntiKnockback = v
    if v then startAntiKB() elseif antiKBLoop then antiKBLoop:Disconnect() end
end)
criarToggle(abaGeral, "GODMODE", 0.52, addY(0.06), 100, 26, Cores.FundoClaro, function(v)
    Config.Godmode = v
...
criarToggle(abaGeral, "REGEN", 0.04, addY(0.06), 100, 26, Cores.FundoClaro, function(v)
    Config.Regen = v
    if v then startRegen() elseif regenLoop then regenLoop:Disconnect() end
end)
criarSlider(abaGeral, "Vel", 0.52, addY(0.06), 120, 26, 1, 20, 5, function(v)
    Config.RegenSpeed = v
end)
addY(0.12)
...
criarToggle(abaJogo, "GERADOR", 0.04, addY(0.06), 130, 26, Cores.FundoClaro, function(v)
    Config.AutoGenerator = v
    if v then startAutoGen() else stopAutoGen() end
end)
criarToggle(abaJogo, "TURBO", 0.52, addY(0.06), 100, 26, Cores.FundoClaro, function(v)
    Config.TurboMode = v
end)
addY(0.10)

-- Modo Gerador
criarBotao(abaJogo, "Ótimo", 0.04, addY(0), 65, 24, Cores.Verde, function()
    Config.GeneratorMode = "great"
    genLabel.Text = "Ótimo"
end)
criarBotao(abaJogo, "Normal", 0.30, addY(0), 65, 24, Cores.Amarelo, function()
    Config.GeneratorMode = "normal"
    genLabel.Text = "Normal"
end)
local genLabel = Instance.new("TextLabel")
genLabel.Size = UDim2.new(0.2, 0, 0, 24)
genLabel.Position = UDim2.new(0.60, 0, addY(0), 0)
...
criarSlider(abaJogo, "Distância", 0.04, addY(0), 250, 26, 5, 50, 25, function(v)
    Config.AutoGeneratorRange = v
end)
addY(0.10)

criarToggle(abaJogo, "SKILL PERF", 0.04, addY(0), 150, 26, Cores.FundoClaro, function(v)
...
criarBotao(abaJogo, "COMPLETAR TUDO", 0.04, addY(0.06), 220, 28, Cores.Azul, function()
...
criarToggle(abaJogo, "AUTO LEAVE", 0.04, addY(0.06), 110, 26, Cores.FundoClaro, function(v)
    Config.AutoLeave = v
    if v then startAutoLeave() else stopAutoLeave() end
end)
criarSlider(abaJogo, "Dist", 0.52, addY(0.06), 110, 26, 5, 30, 15, function(v)
...
criarToggle(abaJogo, "ATIVAR", 0.04, addY(0.06), 80, 26, Cores.FundoClaro, function(v)
    Config.AutoAttack = v
    if v then startAutoAttack() else stopAutoAttack() end
end)
criarSlider(abaJogo, "Alcance", 0.35, addY(0.06), 80, 26, 5, 20, 10, function(v)
    Config.AttackRange = v
end)
criarSlider(abaJogo, "CD", 0.68, addY(0.06), 80, 26, 0.5, 2.0, 1.2, function(v)
...
criarToggle(abaProtecao, "GODMODE", 0.04, addY(0.06), 200, 28, Cores.FundoClaro, function(v)
    Config.Godmode = v
    if v then startGodmode() elseif godLoop then godLoop:Disconnect() end
end)
addY(0.12)
...
criarToggle(abaProtecao, "REGEN", 0.04, addY(0.06), 100, 26, Cores.FundoClaro, function(v)
    Config.Regen = v
    if v then startRegen() elseif regenLoop then regenLoop:Disconnect() end
end)
criarSlider(abaProtecao, "Vel", 0.52, addY(0.06), 120, 26, 1, 20, 5, function(v)
    Config.RegenSpeed = v
end)
addY(0.12)

criarSecao(abaProtecao, "🛡️ DEFESA", addY(0))
criarToggle(abaProtecao, "ANTI-KB", 0.04, addY(0.06), 200, 28, Cores.FundoClaro, function(v)
    Config.AntiKnockback = v
    if v then startAntiKB() elseif antiKBLoop then antiKBLoop:Disconnect() end
end)
addY(0.12)
...
criarToggle(abaProtecao, "NOCLIP", 0.04, addY(0.06), 200, 28, Cores.FundoClaro, function(v)
    Config.Noclip = v
    applyNoclip()
end)
addY(0.12)
...
local function ajustarCanvas(aba)
    ...
    aba.CanvasSize = UDim2.new(0, 0, 0, maxY + 50)
end
    ...
    if map then
        for _, obj in ipairs(map:GetDescendants()) do
            if obj:IsA("Model") and (obj.Name:find("Pallet") or obj.Name:find("Palete")) then
                ...
                if dropped then removeHighlight(obj) else createHighlight(obj, Color3.fromRGB(255,255,0)) end
            end
        end
    end
end
...
print("========================================")
print("✅ ABD HUB – VERSÃO BONITA")
print("📱 Layout organizado para celular")
