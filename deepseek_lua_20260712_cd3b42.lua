...
-- Auto Generator corrigido: só funciona perto do gerador + modo PERFEITO
...
local Config = {
    ...
    AutoFeatures = {
        AutoGenerator = false,
        GeneratorMode = "perfect",  -- "perfect", "great" ou "normal"
        AutoGeneratorRange = 25,     -- Distância máxima para atuar
...
-- =============================================
-- AUTO GENERATOR CORRIGIDO (COM DISTÂNCIA E PERFEITO)
-- =============================================
task.spawn(function()
    while task.wait(0.15) do
        if Config.AutoFeatures.AutoGenerator then
            ...
            local map = Workspace:FindFirstChild("Map")
            if not map then goto continue end
            ...
            -- Se estiver longe do gerador, NÃO FAZ NADA
            if nearestDist > Config.AutoFeatures.AutoGeneratorRange then
                goto continue
            end
            
            -- Executa o Auto Generator (perfeito)
            safeCall(function()
                ...
                for _, obj in ipairs(map:GetDescendants()) do
                    if obj:IsA("Model") and obj.Name == "Generator" then
                        -- Só atua em geradores próximos
                        local genPart = obj:FindFirstChildWhichIsA("BasePart")
                        if genPart then
                            local dist = (genPart.Position - hrp.Position).Magnitude
                            if dist > Config.AutoFeatures.AutoGeneratorRange then
                                goto next_obj
                            end
                        end
                        ...
                        for _, point in ipairs(obj:GetChildren()) do
                            if point.Name:find("GeneratorPoint") then
                                pcall(function()
                                    ...
                                    -- MODO PERFEITO: sempre "success" com valor máximo
...
GameplayTab:CreateToggle({Name = "Auto Completar Geradores", CurrentValue = false, Flag = "AutoGenerator", Callback = function(v)
    ...
    notify("Auto Gerador", v and "Ativado (Perfeito)" or "Desativado", 2)
end})
...
GameplayTab:CreateLabel("⚠️ O Auto Generator só funciona se você estiver dentro dessa distância.")
GameplayTab:CreateLabel("✅ Modo PERFEITO: sempre acerta o skill check no máximo!")
...
print("========================================")
print("✅ ABD HUB – Auto Generator com Distância e PERFEITO")
print("📱 Mobile:", isMobile and "SIM" or "NÃO")
print("⚡ Auto Generator só funciona perto do gerador")
print("🎯 Modo PERFEITO: skill check sempre no máximo")
print("========================================")