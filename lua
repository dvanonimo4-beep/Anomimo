--[[ ABD HUB – CORRIGIDO (Noclip, Godmode, Speed 17-100) ]]
...
print("🚀 Carregando Abd Hub (Corrigido)...")
...
-- =============================================
local Config = {
    Velocidade = 17,  -- MÍNIMO 17
    Noclip = false,
    Godmode = false,
    Regen = false,
    RegenSpeed = 5,
    AntiKnockback = false,  -- Mantido, mas não está causando problema
    ESP = {
        Assassino = false,
        Sobrevivente = false,
        Gerador = false,
    ...
    AtivarPoder = false,
    DesabilitarParticulas = false,
    GraficosBaixos = false,
    DesabilitarSombras = false,
    ReduzirDistancia = false,
    ControlesToque = false,
    TamanhoBotao = 80,
}
...
-- =============================================
-- FUNÇÕES DE PROTEÇÃO (CORRIGIDAS)
-- =============================================
local function applySpeed()
    local hum = myChar:FindFirstChildOfClass("Humanoid")
    if hum then
        -- Garantir que a velocidade fique entre 17 e 100
        Config.Velocidade = math.clamp(Config.Velocidade, 17, 100)
...
local function updateESP()
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Team then
            ...
            if tn == "Killer" and Config.ESP.Assassino then
                createHighlight(player.Character, Color3.fromRGB(255,0,0))
            elseif tn == "Survivors" and Config.ESP.Sobrevivente then
    ...
    for _, obj in ipairs(map:GetDescendants()) do
        if obj:IsA("Model") then
            if obj.Name == "Generator" and Config.ESP.Gerador then
                createHighlight(obj, Color3.fromRGB(203,132,66))
            elseif obj.Name == "Gate" and Config.ESP.Portao then
                createHighlight(obj, Color3.fromRGB(255,255,255))
            elseif obj.Name == "Hook" and Config.ESP.Gancho then
                createHighlight(obj, Color3.fromRGB(255,0,0))
            elseif obj.Name == "Palletwrong" and Config.ESP.Palete then
                createHighlight(obj, Color3.fromRGB(255,255,0))
            elseif obj.Name == "Window" and Config.ESP.Janela then
                createHighlight(obj, Color3.fromRGB(173,216,230))
            elseif obj.Name:find("Pumpkin") and Config.ESP.Abobora then
        ...
        if point.Name:find("GeneratorPoint") then
            for i = 1, repeats do
                pcall(function()
                    ...
                    skill:FireServer("success", 1, target, point)
                end)
            end
        end
    end
...
-- =============================================
-- ATIVAR PODER DO ASSASSINO
-- =============================================
local function activateKillerPower()
    safeCall(function()
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        if remotes then
            ...
            if killerRemotes then
                ...
                if killerFolder then
                    ...
                    if activate then
                        activate:FireServer()
                        notify("Poder Ativado", "Poder do assassino acionado")
                    end
                end
            end
        end
...
local function applyPerformanceSettings()
    ...
    if Config.DesabilitarParticulas then
        safeCall(function()
    ...
    if Config.GraficosBaixos then
        safeCall(function() settings().Rendering.QualityLevel = Enum.QualityLevel.Level01 end)
    end
    if Config.DesabilitarSombras then
        safeCall(function() lighting.GlobalShadows = false end)
    end
    if Config.ReduzirDistancia then
        safeCall(function()
...
floatBtn.Position = UDim2.new(0.02, 0, 0.82, 0)
floatBtn.BackgroundColor3 = Color3.fromRGB(255, 60, 60)
...
panel.Position = UDim2.new(0.5, -130, 0.4, -140)
...
abaContainer.Size = UDim2.new(1, 0, 0, 22)
abaContainer.Position = UDim2.new(0, 0, 0, 26)
abaContainer.BackgroundTransparency = 1
abaContainer.Parent = panel

local abas = {"👁️", "🟢", "🔴", "🛡️", "⚙️"}
local nomesAbas = {"ESP", "Sob", "Ass", "Prot", "Conf"}
local botoesAba = {}
local framesAba = {}

for i, nome in ipairs(abas) do
    local btn = Instance.new("TextButton")
    btn.Size = UDim2.new(0, 48, 1, -2)
...
scroll.ScrollBarImageColor3 = Cores.ScrollBar
scroll.ScrollBarImageTransparency = 0.3
scroll.Parent = panel

-- =============================================
-- FUNÇÕES PARA CRIAR ELEMENTOS COM CALLBACKS
-- =============================================
local function criarToggle(parent, texto, y, varName, callback)
    local btn = Instance.new("TextButton")
    ...
    btn.MouseButton1Click:Connect(function()
        state = not state
        btn.Text = (state and "⬤ " or "◯ ") .. texto .. ": " .. (state and "ON" or "OFF")
        btn.BackgroundColor3 = state and Color3.fromRGB(40, 130, 40) or Color3.fromRGB(40, 40, 60)
        btn.BorderColor3 = state and Color3.fromRGB(0, 255, 100) or Cores.Texto
        if varName then Config[varName] = state end
        if callback then callback(state) end
        print("🔘 " .. texto .. ":", state and "ON" or "OFF")
    end)
    return btn
end

local function criarSlider(parent, texto, y, default, min, max, varName, callback)
    local frame = Instance.new("Frame")
    frame.Size = UDim2.new(0.9, 0, 0, 24)
    ...
    menos.Size = UDim2.new(0, 20, 0, 16)
    ...
    local function atualizar()
        ...
        if varName then Config[varName] = val end
...
for i = 1, 5 do
    table.insert(framesAba, criarAbaFrame())
end
...
-- =============================================
local y = 8
criarToggle(framesAba[1], "Assassino (Vermelho)", y, nil, function(v)
    Config.ESP.Assassino = v
    if Config.ESP.Assassino or Config.ESP.Sobrevivente then startESP() else stopESP() end
end); y = y + 30
criarToggle(framesAba[1], "Sobrevivente (Verde)", y, nil, function(v)
    Config.ESP.Sobrevivente = v
    if Config.ESP.Assassino or Config.ESP.Sobrevivente then startESP() else stopESP() end
end); y = y + 30
criarToggle(framesAba[1], "Gerador (Laranja)", y, nil, function(v) Config.ESP.Gerador = v end); y = y + 30
criarToggle(framesAba[1], "Portão (Branco)", y, nil, function(v) Config.ESP.Portao = v end); y = y + 30
criarToggle(framesAba[1], "Gancho (Vermelho)", y, nil, function(v) Config.ESP.Gancho = v end); y = y + 30
criarToggle(framesAba[1], "Palete (Amarelo)", y, nil, function(v) Config.ESP.Palete = v end); y = y + 30
criarToggle(framesAba[1], "Janela (Azul Claro)", y, nil, function(v) Config.ESP.Janela = v end); y = y + 30
criarToggle(framesAba[1], "Abóbora (Laranja)", y, nil, function(v) Config.ESP.Abobora = v end); y = y + 30
criarToggle(framesAba[1], "Mostrar Distância", y, nil, function(v) Config.ESP.MostrarDistancia = v end); y = y + 30
criarSlider(framesAba[1], "Distância Máxima", y, 500, 50, 1000, nil, function(v) Config.ESP.DistanciaMaxima = v end); y = y + 30
framesAba[1].Size = UDim2.new(1, 0, 0, y + 10)

-- =============================================
-- ABA SOBREVIVENTE
-- =============================================
y = 8
criarToggle(framesAba[2], "Auto Generator", y, nil, function(v)
    Config.AutoGenerator = v
    if v then startAutoGen() else stopAutoGen() end
end); y = y + 30
criarToggle(framesAba[2], "Turbo Mode", y, nil, function(v) Config.TurboMode = v end); y = y + 30
criarSlider(framesAba[2], "Distância", y, 25, 5, 50, nil, function(v) Config.AutoGeneratorRange = v end); y = y + 30
criarToggle(framesAba[2], "Skill Check Perf", y, nil, function(v)
    Config.SkillCheckManual = v
    if v then setupSkillCheck() end
end); y = y + 30
criarToggle(framesAba[2], "Completar Todos", y, nil, function(v)
    if v then completeAllGenerators() end
end); y = y + 30
criarToggle(framesAba[2], "Auto Leave", y, nil, function(v)
    Config.AutoLeave = v
    if v then startAutoLeave() else stopAutoLeave() end
end); y = y + 30
criarSlider(framesAba[2], "Distância", y, 15, 5, 30, nil, function(v) Config.LeaveDistance = v end); y = y + 30
framesAba[2].Size = UDim2.new(1, 0, 0, y + 10)

-- =============================================
-- ABA ASSASSINO
-- =============================================
y = 8
criarToggle(framesAba[3], "Auto Attack", y, nil, function(v)
    Config.AutoAttack = v
    if v then startAutoAttack() else stopAutoAttack() end
end); y = y + 30
criarSlider(framesAba[3], "Alcance", y, 10, 5, 20, nil, function(v) Config.AttackRange = v end); y = y + 30
criarSlider(framesAba[3], "Cooldown", y, 1.2, 0.5, 2.0, nil, function(v) Config.AttackCooldown = v end); y = y + 30
criarToggle(framesAba[3], "Ativar Poder", y, nil, function(v)
    if v then activateKillerPower() end
end); y = y + 30
framesAba[3].Size = UDim2.new(1, 0, 0, y + 10)

-- =============================================
-- ABA PROTEÇÃO (COM NOCLIP E GODMODE CORRIGIDOS)
-- =============================================
y = 8
criarToggle(framesAba[4], "Godmode", y, nil, function(v)
    Config.Godmode = v
    if v then startGodmode() elseif godLoop then godLoop:Disconnect() end
end); y = y + 30
criarToggle(framesAba[4], "Regeneração", y, nil, function(v)
    Config.Regen = v
    if v then startRegen() elseif regenLoop then regenLoop:Disconnect() end
end); y = y + 30
criarSlider(framesAba[4], "Velocidade Regen", y, 5, 1, 20, nil, function(v) Config.RegenSpeed = v end); y = y + 30
criarToggle(framesAba[4], "Anti-Knockback", y, nil, function(v)
...
-- NOCLIP (CORRIGIDO)
-- =============================================
criarToggle(framesAba[4], "Noclip", y, nil, function(v)
    Config.Noclip = v
    applyNoclip()
end); y = y + 30
framesAba[4].Size = UDim2.new(1, 0, 0, y + 10)

-- =============================================
-- ABA CONFIG
-- =============================================
y = 8
criarToggle(framesAba[5], "Des. Partículas", y, nil, function(v)
    Config.DesabilitarParticulas = v
    applyPerformanceSettings()
end); y = y + 30
criarToggle(framesAba[5], "Gráficos Baixos", y, nil, function(v)
    Config.GraficosBaixos = v
    applyPerformanceSettings()
end); y = y + 30
criarToggle(framesAba[5], "Des. Sombras", y, nil, function(v)
    Config.DesabilitarSombras = v
    applyPerformanceSettings()
end); y = y + 30
criarToggle(framesAba[5], "Reduzir Dist.", y, nil, function(v)
    Config.ReduzirDistancia = v
    applyPerformanceSettings()
end); y = y + 30
criarToggle(framesAba[5], "Controles Toque", y, nil, function(v)
    Config.ControlesToque = v
end); y = y + 30
criarSlider(framesAba[5], "Tamanho Botão", y, 80, 40, 120, nil, function(v)
    Config.TamanhoBotao = v
end); y = y + 30
framesAba[5].Size = UDim2.new(1, 0, 0, y + 10)

-- =============================================
-- LÓGICA DE TROCA DE ABA
-- =============================================
local function mostrarAba(indice)
    for i, f in ipairs(framesAba) do
        f.Visible = (i == indice)
    end
    local frameAtual = framesAba[indice]
    if frameAtual then
        scroll.CanvasSize = UDim2.new(0, 0, 0, frameAtual.Size.Y.Offset + 10)
    end
    for i, btn in ipairs(botoesAba) do
        btn.TextColor3 = (i == indice) and Cores.Borda or Cores.TextoClaro
    end
end
...
if Config.ESP.Assassino or Config.ESP.Sobrevivente then startESP() end
if Config.Godmode then startGodmode() end

print("========================================")
print("✅ ABD HUB CORRIGIDO!")
print("⚡ Speed: mínimo 17 / máximo 100")
print("🚪 Noclip: funcionando")
print("♾️ Godmode: funcionando")
print("🔴 Bolinha no canto inferior esquerdo")
print("========================================")
